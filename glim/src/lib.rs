...
impl Background {
    ...
    pub fn gray(ctx: &VulkanContext, width: u32, height: u32, gray_value: f32) -> Background {
        ...
        for _ in 0..(texels_per_face * 6) {
            pixels.push(gray_value); // R
            pixels.push(gray_value); // G
            pixels.push(gray_value); // B
            pixels.push(1.0); // A
        }
...
fn render_preview(app: &mut Glim) {
    ...
    let background = Background::gray(&app.vk, 128, 128, 0.5);
